...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 30 20:01:38 2023
...
def my_inter_cluster_distance(X):
...
def my_calinski_harabasz_score(X):
    labels = np.unique(X['labels'])
    n_clusters = labels.size
    centroids  = pd.DataFrame()
...
def tsne_class_conv(df):
    unique_labels = np.unique(df['labels'])
    
    for labl in unique_labels:
        sub_df  = df[df['labels'] == labl]
        cov_mat = np.cov(sub_df[['tsne_x', 'tsne_y']].to_numpy().T)
...
def labels_to_int(df):
    if df['labels'] == 'REFUGE':
        val = 0
    elif df['labels'] == 'Magrabi':
        val = 1
    elif df['labels'] == 'MESSIDOR':
        val = 2
    elif df['labels'] == 'BinRushed':
        val = 3
    elif df['labels'] == 'Drishti-GS':
        val = 4
    elif df['labels'] == 'RWD':
        val = 5
    return val
...
# =========================================
ic_distance = my_inter_cluster_distance(df)
print('Mean inter-cluster distance: \n'); print(ic_distance); print('='*30)

# # =========================================
sd = get_cluster_SD(df)
print('Standard deviation in (x, y) directions: \n'); print(sd); print('='*30)

# =========================================
# calinski_harabasz_score(t_sne, labels)
ch_score = my_calinski_harabasz_score(df)

# =========================================
silhouette_scores = silhouette_score_per_cluster(df[['tsne_x', 'tsne_y']].to_numpy(), df['labels'].to_numpy())
